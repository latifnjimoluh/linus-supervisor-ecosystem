// controllers/chatbot/askStream.js
...
const STREAM_SPLIT = (process.env.STREAM_SPLIT || "word").toLowerCase(); // chunk|word|char
const STREAM_DELAY_MS = Number(process.env.STREAM_DELAY_MS || 0);
const HEARTBEAT_MS = Number(process.env.STREAM_HEARTBEAT_MS || 15000);

const splitText = (t) => {
  if (STREAM_SPLIT === "char") return [...t];
  if (STREAM_SPLIT === "word") return t.split(/(\s+)/);
  return [t];
};
const sleep = (ms) => new Promise(r => setTimeout(r, ms));

/** ------------ router ------------ **/
function createAskStreamRouter({ model, systemPrompt, logChat }) {
    ...
    try {
      ...
      try {
        logChat?.({
          ...
          durationMs, ok: true, mode: `stream:${STREAM_SPLIT}`,
