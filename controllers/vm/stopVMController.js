const { stopVM } = require("../../utils/proxmoxService");
...
exports.stopVM = async (req, res) => {
  ...
  try {
    await stopVM({ vmId: vm_id, node, apiUrl, tokenId, tokenName, tokenSecret });
    return res.status(200).json({ message: "✅ Arrêt de la VM demandé" });
  } catch (error) {
    console.error("❌ Erreur arrêt VM:", error.message);
    return res.status(500).json({ message: "❌ Échec de l'arrêt de la VM", error: error.message });
  }
};
