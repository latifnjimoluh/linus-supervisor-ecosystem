...
[ "$EUID" -ne 0 ] && echo "❌ Ce script doit être exécuté en root (via Cloud-Init ou Terraform remote-exec)." && exit 1
...
# 2. Configuration globale de Bind9
...
# 3. Configuration de la zone camer.cm
...
# 6. Redémarrage du service sans sudo
