...
echo "🌐 Déploiement du serveur Web Camer-Web (web.camer.cm)..."
...
echo "📁 Création du site web /var/www/web.camer.cm"
sudo mkdir -p /var/www/web.camer.cm

echo "📝 Création de la page d’accueil personnalisée..."
sudo tee /var/www/web.camer.cm/index.html > /dev/null <<EOF
<!DOCTYPE html>
<html lang=\"fr\">
<head>
    <meta charset=\"UTF-8\">
...
<body style=\"font-family: sans-serif; text-align: center; margin-top: 100px;\">
    ...
    <p>🌐 Vous êtes sur : <strong>web.camer.cm</strong></p>
    <p>📍 IP : <strong>192.168.24.130</strong></p>
...
echo "🔧 Configuration NGINX pour web.camer.cm..."
sudo tee /etc/nginx/sites-available/web.camer.cm > /dev/null <<EOF
server {
    listen 80;
    server_name web.camer.cm;

    access_log /var/log/nginx/web.camer.cm.access.log;
    error_log /var/log/nginx/web.camer.cm.error.log;

    root /var/www/web.camer.cm;
    index index.html;
...
sudo ln -s /etc/nginx/sites-available/web.camer.cm /etc/nginx/sites-enabled/
...
echo
echo "✅ Camer-Web est prêt. Teste http://web.camer.cm depuis le reverse proxy ou le client interne."
