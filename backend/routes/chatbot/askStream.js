...
module.exports = function createAskStreamRouter({ model, SYSTEM_PROMPT, logChat }) {
  const router = express.Router();

  router.post("/", async (req, res) => {
    const started = Date.now();
    const rid = req.id || randomUUID().slice(0, 10);
