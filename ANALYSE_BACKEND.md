...
| POST | /api/permissions/assign | userPermissionController.assignPermissionsToRole | verifyToken, isSuperAdmin | RolePermission | ✅ |
| GET | /api/permissions/role/:role_id | userPermissionController.getPermissionsByRole | verifyToken | Permission, Role | ✅ |
| GET | (non monté) /user-logs/users/:id | userActionLogController.getLogsByUser | verifyToken, checkPermission('userLogs.view') | UserActionLog | ⚠️ non monté |
| POST | /api/vm/deploy-vm | deployVMController.deployVMDirect | verifyToken, checkPermission('vm.deploy') | Deployment, ConfigTemplateService, InitScript, MonitoringScript, MonitoringService, UserSetting | ✅ |
...
| POST | /api/services/config-template | templateServiceController.configureService | verifyToken, checkPermission('serviceConfig.configure') | ConfigTemplate, ConfigTemplateService | ✅ |
| POST | /api/monitoring/generate | generateMonitoringDNSController.generateMonitoringScript | verifyToken, checkPermission('monitoringScript.generate') | MonitoringScript, ServiceTemplate | ✅ |
| POST | /api/monitoring/monitoring-services/generate | generateMonitoringServiceController.generateMonitoringServiceScript | verifyToken, checkPermission('monitoringService.generate') | MonitoringService, ServiceTemplate | ✅ |
| POST | /api/init-scripts/generate | generateInitScriptController.generateInitScript | verifyToken, checkPermission('initScript.generate') | InitScript | ✅ |
...
- **RolePermission** : aucune route dédiée pour retirer des permissions.
- **UserActionLog** : le routeur n’est pas monté dans `routes/index.js`.
- **InitScript**, **MonitoringScript**, **MonitoringService** : seules des routes de création existent (`POST /.../generate`). Pas de liste, détail, mise à jour ou suppression.
- **ConfigTemplate** et **ConfigTemplateService** : uniquement création (`POST`), aucune lecture ou modification.
- **Deployment**, **Delete**, **ServiceStatus**, **StatusSnapshot**, **VMInstance** : aucun endpoint REST exposé (liste ou détail).
- **SupervisionStatus** et **ServiceTemplate** : modèles attendus mais absents ; toute route correspondante est donc inopérante.
...
- Duplication ou fichiers résiduels : `templateServixeController.js`, `monitoringServiceController.js` doublonnent d’autres contrôleurs.
...
- Certaines routes utilisent des modèles inexistants (`SupervisionStatus`, `ServiceTemplate`), ce qui provoque des erreurs à l’exécution.
...
| Modèles Sequelize | `SupervisionStatus` et `ServiceTemplate` inexistants mais utilisés ; modèles comme `Deployment` ou `VMInstance` sans exposition API. |
